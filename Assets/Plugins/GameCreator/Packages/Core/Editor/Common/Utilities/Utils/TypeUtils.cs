...
namespace GameCreator.Editor.Common
{
    public static class TypeUtils
    {
        ...
        public static Tree<Type> GetTypesTree(Type typeBase)
        {
            Tree<Type> tree = Tree<Type>.Create();
            ...
            foreach (Type type in types)
            {
                ...
                Tree<Type> subTree = tree;
                foreach (string section in paths)
                {
                    if (!subTree.Children.TryGetValue(section, out Tree<Type> child))
                    {
                        child = subTree.AddChild(new Tree<Type>(section, null));
                    }

                    subTree = child;
                }

                subTree.AddChild(new Tree<Type>(name, type));
            }

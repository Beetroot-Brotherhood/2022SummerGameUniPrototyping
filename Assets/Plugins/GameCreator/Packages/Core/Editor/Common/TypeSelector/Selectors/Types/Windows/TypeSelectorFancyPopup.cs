    ...
    public class TypeSelectorFancyPopup : EditorWindow
    {
        ...
        private void SetupHead()
        {
            ...
            this.m_SearchField.RegisterValueChangedCallback(changeEvent =>
            {
                ...
                Tree<Type> pagesTree = new Tree<Type>(changeEvent.newValue, null);
                
                foreach (Type type in types)
                {
                    string typeName = type.ToString();
                    pagesTree.AddChild(new Tree<Type>(typeName, type));
                }
                
                TypePage typePage = new TypePage(pagesTree, true);
        ...
        private void SetupBody()
        {
            ...
            this.SetupNavigationPage(rootPage, false);
        }

        private void SetupNavigationPage(TypePage typePage, bool transition)
        {
            VisualElement page = this.CreatePage(typePage, transition, false);
        ...
        private void SetupSearchPage(TypePage typePage)
        {
            ...
            this.m_ContentSearch = this.CreatePage(typePage, false, true);
            this.m_Body.Add(this.m_ContentSearch);
        }

        private VisualElement CreatePage(TypePage typePage, bool transition, bool isSearch)
        {
            ...
            if (isSearch || this.m_ContentStack.Count > 0)
            {
                switch (isSearch)
                {
                    case true:
                        header.RegisterCallback<MouseDownEvent>(
                            mouseEvent => this.m_SearchField.value = string.Empty
                        );
                        break;
                ...
                headerTitle.Add(new Image {image = ICON_CHEVRON_L.Texture});                
            }
        ...
        private void ContentChooseItem(IEnumerable<object> list)
        {
            foreach (TypeNode node in list)
            {
                ...
                {
                    case TypeNodeFolder nodeFolder:
                        this.SetupNavigationPage(nodeFolder.Page, true);
                        break;
        ...
        private void PreviousPage(MouseDownEvent mouseEvent)
        {
        ...
        private void Transition(VisualElement element, float t)
        {

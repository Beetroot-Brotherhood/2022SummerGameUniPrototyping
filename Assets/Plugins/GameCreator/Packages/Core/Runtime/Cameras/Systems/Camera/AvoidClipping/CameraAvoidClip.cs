...
namespace GameCreator.Runtime.Cameras
{
    ...
    public class CameraAvoidClip
    {
        ...
        {
            this.m_HitBuffer = new RaycastHit[10];
            this.m_CurrentDistance = this.m_MinDistance + this.m_Radius;
        }

        // UPDATE METHODS: ------------------------------------------------------------------------

        public Vector3 Update(TCamera camera, Transform target)
        {
            if (!this.m_Enabled || target == null || camera.transform.position == target.position)
            {

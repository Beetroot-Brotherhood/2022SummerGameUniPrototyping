using System;
using GameCreator.Runtime.Common;
using UnityEngine;

namespace GameCreator.Runtime.VisualScripting
{
    [Title("On Trigger Exit Tag")]
    [Category("Physics/On Trigger Exit Tag")]
    [Description("Executed when a game object with a Tag exists the Trigger collider")]

    [Parameter("Tag", "A string that represents a group of game objects")]
    
    [Image(typeof(IconTag), ColorTheme.Type.Red)]
    
    [Keywords("Pass", "Through", "Touch", "Collision", "Collide")]

    [Serializable]
    public class EventTriggerExitTag : Event
    {
        [SerializeField] private TagValue m_Tag = new TagValue();
        
        // METHODS: -------------------------------------------------------------------------------
        
        protected internal override void OnAwake(Trigger trigger)
        {
            base.OnAwake(trigger);
            trigger.RequireRigidbody();
        }
        
        protected internal override void OnTriggerExit3D(Trigger trigger, Collider collider)
        {
            base.OnTriggerExit3D(trigger, collider);
            
            if (!collider.gameObject.CompareTag(this.m_Tag.Value)) return;
            _ = this.m_Trigger.Execute(collider.gameObject);
        }
        
        protected internal override void OnTriggerExit2D(Trigger trigger, Collider2D collider)
        {
            base.OnTriggerExit2D(trigger, collider);
            
            if (!collider.gameObject.CompareTag(this.m_Tag.Value)) return;
            _ = this.m_Trigger.Execute(collider.gameObject);
        }
    }
}